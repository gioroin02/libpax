#ifndef PAX_CORE_FORMAT_INTEGER_C
#define PAX_CORE_FORMAT_INTEGER_C
...
pax_integer_digits(paxiword value, Pax_Format_Radix radix, Pax_Format_Flag flags)
{
    ...
    paxuword temp   = pax_magn_integer(value);
    paxb8    sign   = pax_sign_integer(value) < 0 ? 1 : 0;
    ...
    if (sign != 0 || (flags & PAX_FORMAT_FLAG_PLUS) != 0)
        digits += 1;
...
{
    paxiword size  = pax_integer_digits(value, radix, flags);
    paxuword width = pax_magn_format_radix(radix);
    paxuword temp  = pax_magn_integer(value);
    paxb8    sign  = pax_sign_integer(value) < 0 ? 1 : 0;
    ...
    if (sign != 0) memory[index] = PAX_ASCII_MINUS;
...
{
    paxiword index = 0;
    paxiword width = pax_magn_format_radix(radix);
    paxiword temp  = 0;
    paxb8    sign  = 0;
    ...
    switch (memory[index]) {
        ...
        case PAX_ASCII_MINUS: sign = 1; index += 1; break;
    ...
    for (; index < length; index += 1) {
        ...
        switch (sign) {
            case 0: {
                if (temp > (PAX_IWORD_MAX - digit) / width)
                    return 0;
            ...
            default: {
                if (temp < (PAX_IWORD_MIN + digit) / width)
                    return 0;
...
pax_string8_from_integer(Pax_Arena* arena, paxiword value, Pax_Format_Radix radix, Pax_Format_Flag flags)
{
    ...
    paxiword length = pax_integer_digits(value, radix, flags);

    paxu8* memory =
        pax_arena_reserve(arena, paxu8, length + 1);

    paxiword size = pax_memory8_write_integer(
...
pax_integer_from_string8(Pax_String8 string, paxiword* value, Pax_Format_Radix radix, Pax_Format_Flag flags)
{
    ...
    paxiword size = pax_memory8_read_integer(
        memory, length, value, radix, flags);

    return size == length ? 1 : 0;
}

#endif // PAX_CORE_FORMAT_INTEGER_C
